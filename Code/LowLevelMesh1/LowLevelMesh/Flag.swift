...
class ClothSimMetalNode {
    ...
    init(device: MTLDevice, width: uint, height: uint) {
        ...
        self.positionBuffer = vertexBuffer2!
...
class MetalClothSimulator {
    ...
    func deform(_ mesh: ClothSimMetalNode, simData: SimulationData) {
        ...
        clothSimCommandEncoder?.setBuffer(mesh.positionBuffer, offset: 0, index: 1)
        ...
        // Pass 2: updateNormal (swap direction)
        clothSimCommandEncoder?.setComputePipelineState(pipelineStateNormalUpdate)
        clothSimCommandEncoder?.setBuffer(mesh.positionBuffer, offset: 0, index: 0)
        ...
        clothSimCommandEncoder?.setBuffer(mesh.positionBuffer, offset: 0, index: 0)
        clothSimCommandEncoder?.setBuffer(mesh.normalBuffer, offset: 0, index: 1)
        clothSimCommandEncoder?.setBuffer(mesh.uvBuffer, offset: 0, index: 2)
        
        clothSimCommandEncoder?.setBuffer(writevb, offset: 0, index: 3)//position
        clothSimCommandEncoder?.setBuffer(writevb, offset: MemoryLayout<SIMD3<Float>>.size * mesh.vertexCount, index: 4)//normal
        clothSimCommandEncoder?.setBuffer(writevb, offset: MemoryLayout<SIMD3<Float>>.size * mesh.vertexCount * 2, index: 5)//uv
        clothSimCommandEncoder?.dispatchThreadgroups(threadgroupsPerGrid, threadsPerThreadgroup: threadsPerThreadgroup)
